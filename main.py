...
def smart_ai_reply(msg):
    ...
    if "review" in m: return {"reply":"⭐ 4.9/5 from 2,340+ Kenyans!","action":None}
    if any(x in m for x in ["hello","hi","jambo"]): return {"reply":"Jambo! Karibu LONMA ORBIT! 🇰🇪","action":None}
    return {"reply":"Try: unga, sukari, mafuta","action":None}
...
def make_logo_svg():
    return b'''<svg xmlns="http://www.w3.org/2000/svg" width="200" height="80" viewBox="0 0 200 80"><rect width="200" height="80" fill="#0A8EA8" rx="12"/><text x="100" y="38" font-size="32" font-weight="900" text-anchor="middle" fill="white" font-family="Arial">LO</text><text x="100" y="62" font-size="12" font-weight="800" text-anchor="middle" fill="white" font-family="Arial">LONMA ORBIT</text></svg>'''
...
<img src="/logo.png" style="width:120px;border-radius:16px;margin:20px auto;display:block;background:#fff;padding:5px"><h1>LONMA ORBIT</h1><div class="card">⭐4.9 by 2,340+ Kenyans. Kajiado HQ.</div><a href="/" style="color:#fff">← Back</a></body></html>""")
...
.header{position:sticky;top:0;z-index:40;background:#0A8EA8}
.h-top{display:flex;justify-content:space-between;align-items:center;padding:10px 16px}
.logo-img{height:52px;border-radius:12px;background:#fff;padding:3px}
.rating-badge{background:#fff;color:#0A8EA8;padding:8px 12px;border-radius:100px;font-size:11px;font-weight:800}
.h-loc{padding:8px 16px 12px;color:#fff}.trust{display:flex;gap:8px;padding:10px 16px;overflow-x:auto}.trust::-webkit-scrollbar{display:none}
.trust-item{white-space:nowrap;background:#151A27;border:1px solid var(--border);padding:8px 12px;border-radius:100px;font-size:11px;font-weight:700;flex-shrink:0;color:#fff}
...
<div class="header"><div class="h-top"><img src="/logo.png" class="logo-img" alt="LONMA ORBIT"><div class="rating-badge">⭐ 4.9 • 2.3k</div></div><div class="h-loc"><b style="font-size:13px">Serving Kenyans • 30min delivery</b><br><small style="font-size:11px;opacity:0.9">Kajiado HQ • Trusted by Wakenya</small></div></div>
...
<div style="padding:20px 16px 110px;text-align:center"><img src="/logo.png" style="height:60px;border-radius:12px;margin-bottom:10px;background:#fff;padding:4px"><p style="font-size:10px;color:#475569">© 2026 LONMA ORBIT • <a href="/terms" style="color:#94A3B8">Terms</a></p></div>
