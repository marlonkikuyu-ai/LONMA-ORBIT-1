...
def smart_ai_reply(message, cart_count=0):
    ...
    if any(w in msg for w in ["hello","hi","hey","jambo"]): return "Hello! 👋 I'm LONMA AI!"
    if "help" in msg: return "I can help:\n🛒 Find cheapest\n💰 Compare prices\n🏍️ Delivery info\n💳 Pay M-Pesa or Cash"
    if "deliver" in msg: return "Yes! 30 mins 🏍️\nKajiado, Kitengela, Rongai\nFee KES 100"
    if "pay" in msg: return "💚 M-Pesa STK Push\n💵 Cash on Delivery\nBoth work!"
    return "Try: Help me, Will you deliver?, How to pay?"

# CREATE PLACEHOLDER IMAGES THAT ALWAYS RETURN 200
def make_svg(text, emoji, bg="#FFFFFF"):
    svg = f'''<svg xmlns="http://www.w3.org/2000/svg" width="300" height="300" viewBox="0 0 300 300">
    <rect width="300" height="300" fill="{bg}" rx="20"/>
    <text x="150" y="130" font-size="80" text-anchor="middle" dominant-baseline="middle">{emoji}</text>
    <text x="150" y="220" font-size="16" font-weight="800" text-anchor="middle" fill="#0B0E14" font-family="Arial">{text}</text>
    </svg>'''
    return svg.encode()

@app.get("/", response_class=HTMLResponse)
async def index():
    return HTMLResponse(open(__file__).read().split("HTML_START")[1].split("HTML_END")[0] if "HTML_START" in open(__file__).read() else get_html())
...
async def product_img(name: str):
    ...
    svg = make_svg(text, emoji, "#FFFFFF")
    return Response(content=svg, media_type="image/svg+xml")

@app.get("/favicon.ico")
async def fav():
    svg = make_svg("LONMA","🛒","#0A8EA8")
    return Response(content=svg, media_type="image/svg+xml")
