...
def smart_ai_reply(msg, cart_count=0):
    m=msg.lower()
    if "hello" in m or "hi" in m or "jambo" in m: return "Jambo! Karibu LONMA ORBIT - Supermarket for Kenyans! 🇰🇪"
    if "help" in m: return "Supermarket items for Kenyans:\n🛒 Unga, Sukari, Mafuta, Mchele\n🥛 Maziwa, Mkate\n🏍️ 30min, KES 100\n💳 M-Pesa & Cash"
    if "deliver" in m: return "Yes! Serving Kenyans in Kajiado, Kitengela, Rongai. 30 mins, KES 100."
    if "pay" in m: return "Lipa na M-Pesa STK or Cash on Delivery"
    return "Try: Nipe unga, Sukari iko?, Mafuta?"
...
<p><small>Serving Kenyans - Supermarket Items Delivery</small></p>
<div class="card"><h2>1. Serving Kenyans</h2><p>LONMA ORBIT delivers supermarket items for Kenyans: Unga, Sukari, Mafuta, Mchele, Maziwa, Mkate. Partner: Naivas, Quickmart, Carrefour.</p></div>
<div class="card"><h2>2. Delivery</h2><p>30min avg, KES 100 fee. Free delivery KARIBU30. Areas: Kajiado, Kitengela, Rongai, 15km.</p></div>
<div class="card"><h2>3. Payments</h2><p>M-Pesa STK Push & Cash. Lipa na M-Pesa.</p></div>
<div class="card"><h2>4. Returns</h2><p>Check at delivery. Sealed supermarket items return 24h.</p></div>
<div class="card"><h2>5. Contact</h2><p>support@lonmaorbit.co.ke - Kajiado - Serving Kenyans</p></div>
</body></html>
    """)
...
.card-body h4{font-size:13px;font-weight:800;margin:4px 0 6px;height:36px;overflow:hidden;line-height:1.25}
...
<div class="header"><div class="h-top"><div class="logo">LONMA ORBIT 🇰🇪</div><div class="icons"><div class="ic">🌙</div><div class="ic">🔍</div><div class="ic">🔔</div></div></div><div class="h-loc"><div class="loc-icon">🇰🇪</div><div><b>Serving Kenyans • Supermarket Items</b><br><small id="userStatus">Karibu! Login</small></div><div style="margin-left:auto;font-size:20px;opacity:0.6" onclick="openProfile()">›</div></div><div class="search-wrap"><div class="search-box">🔍<input id="search" placeholder="Search unga, sugar, oil..." oninput="searchProd()"></div><div class="filter">☰</div></div></div>
...
<div style="background:#151A27;border:1px solid var(--border);border-radius:20px;padding:16px;margin-bottom:16px"><b>🇰🇪 Serving Kenyans</b><p style="font-size:11px;color:var(--muted);margin-top:6px">Supermarket items: Unga, Sukari, Mafuta, Mchele, Maziwa, Mkate for every Kenyan home. Kajiado • Kitengela • Rongai</p></div>
...
<div id="aiChat"><div class="ai-h"><div><b>LONMA AI 🇰🇪</b><div style="font-size:11px;opacity:0.9">Serving Kenyans</div></div><div onclick="toggleAI()" style="width:36px;height:36px;background:rgba(255,255,255,0.2);border-radius:12px;display:flex;align-items:center;justify-content:center">✕</div></div><div class="ai-msgs" id="aiMsgs"><div class="m b">Jambo! Karibu! 🇰🇪 Supermarket items for Kenyans: Unga, Sukari, Mafuta, Mchele, Maziwa, Mkate</div></div><div class="ai-in"><input id="aiInput" placeholder="Ask..." onkeypress="if(event.key==='Enter') sendAI()"><button onclick="sendAI()">Send</button></div></div>
...
<button class="btn btn-mpesa" id="placeBtn" onclick="checkout()">💚 Pay M-Pesa</button><button class="btn btn-cash" id="cashBtn" onclick="checkoutCash()" style="display:none">💵 Cash on Delivery</button><div id="status" style="text-align:center;font-size:11px;font-weight:700;margin-top:10px;color:var(--muted)"></div></div></div></div>
...
async def product_img(name: str):
    maps={"flour":("JOGOO UNGA","🌽"),"sugar":("MUMIAS SUKARI","🍚"),"oil":("FRESH FRI","🫒"),"milk":("BROOKSIDE","🥛"),"bread":("MKATE","🍞"),"rice":("MCHELE","🍚"),"coke":("COCA COLA","🥤"),"omo":("OMO","🧴"),"tomato":("NYANYA","🍅"),"soap":("GEISHA","🧼")}
