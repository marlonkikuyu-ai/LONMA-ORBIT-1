...
async def home():
    ...
    header{{background:#0A8EA8;padding:12px;display:flex;justify-content:center;align-items:center;position:sticky;top:0}}
    ...
    .card{{background:#fff;border-radius:12px;padding:12px;text-align:center}}
...
async def stk(req: Request):
    try:
        ...
        token=get_token()
        if not token:
            return {"error": "M-Pesa keys not set in Render. Go to Dashboard > Environment", "ResponseCode": "1"}
