...
def ai_reply(m):
    m=m.lower()
    if "flour" in m:
        return "Ajab Flour 2kg KES 175 Naivas (cheapest 172 Carrefour). 50 packs left."
    if "milk" in m:
        return "Brookside Milk KES 65 Naivas, 62 Chandarana. Fresh daily!"
    if "rider" in m:
        return "Delivery 30min Kajiado/Kitengela/Rongai. Fee KES 100. 3 riders online."
    if "dark" in m:
        return "Click moon icon top right for dark mode!"
    if "whatsapp" in m:
        return "Click Profile > Login with WhatsApp. Enter 2547... number."
    if "hello" in m or "hi" in m:
        return "Hello! I am LONMA AI with real images, WhatsApp login and dark mode."
...
async def index():
    html = '''
<!DOCTYPE html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1"><meta name="theme-color" content="#0A8EA8"><title>LONMA ORBIT</title>
...
.hero h2{font-size:17px;line-height:1.2;font-weight:800} .hero p{font-size:11px;opacity:0.9;margin-top:4px} .hero-btn{background:#fff;color:#0A8EA8;padding:10px 16px;border-radius:24px;font-weight:800;font-size:11px}
.chips{display:flex;gap:8px;overflow-x:auto;padding:8px 14px} .chips::-webkit-scrollbar{display:none}
.chip{white-space:nowrap;padding:8px 14px;border-radius:20px;background:var(--card);border:1px solid var(--border);font-size:11px;font-weight:700;cursor:pointer} .chip.active{background:#111;color:#fff;border-color:#111}
.cats{display:flex;gap:12px;overflow-x:auto;padding:12px 14px} .cats::-webkit-scrollbar{display:none}
...
.card-img{height:140px;background:#f9fafb;display:flex;align-items:center;justify-content:center;position:relative;overflow:hidden} .card-img img{width:100%;height:100%;object-fit:cover} .emoji-overlay{position:absolute;font-size:32px;background:rgba(255,255,255,0.9);width:48px;height:48px;border-radius:50%;display:flex;align-items:center;justify-content:center}
...
.h-scroll{display:flex;gap:10px;overflow-x:auto;padding-bottom:6px} .h-scroll::-webkit-scrollbar{display:none} .h-card{min-width:160px;background:var(--card);border-radius:18px;padding:10px;box-shadow:0 4px 16px rgba(0,0,0,0.05);border:1px solid var(--border)}
...
.sheet{background:var(--card);width:100%;max-width:500px;margin:0 auto;border-radius:28px 28px 0 0;max-height:88vh;overflow-y:auto} .s-h{padding:18px 16px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid var(--border);position:sticky;top:0;background:var(--card);border-radius:28px 28px 0 0} .s-c{padding:16px}
...
<div class="header">
<div class="h-top"><img src="/logo.png"><div class="icons"><div class="ic-btn" onclick="toggleDark()">🌙</div><div class="ic-btn" onclick="document.getElementById('search').focus()">🔍</div><div class="ic-btn" onclick="showToast('3 new deals!')">🔔</div></div></div>
...
<div class="search"><div class="search-box">🔍<input id="search" placeholder="Search flour, milk, bread..." oninput="searchProd(this.value)"></div><div class="filter-btn">☰</div></div>
</div>

<div class="hero"><div><h2>Free Delivery<br>on First 3 Orders!</h2><p>Use code LONMA30 • Real images + Dark mode</p></div><div class="hero-btn">ORDER NOW</div></div>
...
<div class="section"><div class="sec-head"><h3>Flash Sale</h3><span style="color:#ff3b30">Ends 02:14:33</span></div><div class="h-scroll" id="flash"></div></div>

<div class="bottom">
<div class="tab active" onclick="navTab(this)"><div class="tab-i">🏠</div><b>Home</b></div>
...
<div id="aiChat"><div class="ai-h"><div><b>LONMA AI</b><div style="font-size:11px;opacity:0.85">Real images • WhatsApp • Dark mode</div></div><div onclick="toggleAI()" style="width:32px;height:32px;background:rgba(255,255,255,0.2);border-radius:10px;display:flex;align-items:center;justify-content:center;cursor:pointer">✕</div></div><div class="ai-msgs" id="aiMsgs"><div class="m b">Hello! New features added! Real images, WhatsApp login, Dark mode moon icon top. Try cheapest flour</div></div><div class="ai-in"><input id="aiInput" placeholder="Ask anything..." onkeypress="if(event.key==='Enter') sendAI()"><button onclick="sendAI()">Send</button></div></div>
...
<div style="text-align:center;padding:10px 0 20px"><div style="width:80px;height:80px;background:#0A8EA8;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:36px;color:#fff;margin:0 auto">👤</div><h3 style="margin-top:12px" id="profileName">Welcome to LONMA</h3><small style="color:var(--muted)" id="profilePhone">Login to order faster</small></div>
<div id="notLogged"><input id="waPhone" class="input" value="254" placeholder="WhatsApp Number 254712..."><button class="btn btn-wa" onclick="loginWA()">Login with WhatsApp</button></div>
...
function render(list){
 document.getElementById('grid').innerHTML=list.map(p=>'<div class="card"><div class="card-img"><img src="'+p.img+'" onerror="this.style.display=\\'none\\'"><div class="emoji-overlay">'+p.emoji+'</div><div class="badge">-'+Math.round((p.old-p.price)/p.old*100)+'%</div><div class="heart">♡</div></div><div class="card-body"><div class="store">'+p.store+'</div><h4>'+p.name+'</h4><div class="meta"><span>⭐ '+p.rate+'</span><span>'+p.sold+' sold</span></div><div class="price-row"><div class="price"><b>KES '+p.price+'</b><small>KES '+p.old+'</small></div><button class="add-btn" onclick="add('+p.id+')">+</button></div></div></div>').join('');
}
function renderFlash(){
 document.getElementById('flash').innerHTML=PRODUCTS.slice(0,4).map(p=>'<div class="h-card"><div style="height:80px;border-radius:12px;overflow:hidden;background:#f5f5f5"><img src="'+p.img+'" style="width:100%;height:100%;object-fit:cover" onerror="this.parentElement.innerHTML=\\'<div style=\\\\'font-size:28px;text-align:center;padding:20px\\\\'>'+p.emoji+'</div>\\'"></div><div style="font-size:10px;font-weight:700;margin-top:6px">'+p.name+'</div><div style="display:flex;justify-content:space-between;align-items:center;margin-top:6px"><b style="font-size:12px">KES '+p.price+'</b><button class="add-btn" style="width:26px;height:26px;font-size:14px" onclick="add('+p.id+')">+</button></div></div>').join('');
}
...
function searchProd(q){if(!q){render(PRODUCTS);return} let f=PRODUCTS.filter(p=>p.name.toLowerCase().includes(q.toLowerCase())); render(f)}
function add(id){let p=PRODUCTS.find(x=>x.id==id); cart.push(p); total+=p.price; document.getElementById('cartDot').innerText=cart.length; document.getElementById('cartC').innerText=cart.length; showToast(p.name+' added'); if(navigator.vibrate) navigator.vibrate(30)}
function openCart(){
 let d=document.getElementById('cartItems'); d.innerHTML='';
 if(cart.length==0) d.innerHTML='<p style="text-align:center;padding:24px;color:var(--muted)">Cart empty</p>';
 cart.forEach(c=>d.innerHTML+='<div class="cart-i"><div class="ci"><img src="'+c.img+'" onerror="this.parentElement.innerHTML=\\''+c.emoji+'\\'"></div><div style="flex:1"><h4 style="font-size:12px">'+c.name+'</h4><small style="color:var(--muted)">'+c.store+' • KES '+c.price+'</small></div><b>KES '+c.price+'</b></div>');
...
function navTab(el){document.querySelectorAll('.tab').forEach(t=>t.classList.remove('active')); el.classList.add('active')}
function toggleAI(){document.getElementById('aiChat').classList.toggle('open')}
function toggleDark(){isDark=!isDark; document.body.classList.toggle('dark',isDark); localStorage.setItem('dark',isDark); showToast(isDark?'Dark mode on':'Light mode on')}
function loginWA(){
 ...
 document.getElementById('profilePhone').innerText=phone; document.getElementById('userNameDisplay').innerText='Logged in • '+phone;
 document.getElementById('custPhone').value=phone;
 showToast('Logged in via WhatsApp'); fetch('/user/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phone})});
}
...
async function loadAdmin(){
 ...
 document.getElementById('adminOrders').innerHTML=o.slice(0,8).map(x=>'<div style="background:var(--card);border:1px solid var(--border);border-radius:14px;padding:10px;margin-bottom:6px"><b>'+x.id+'</b> - '+x.status+'<br><small>'+x.location+' • KES '+x.amount+'</small></div>').join('');
}
...
let savedUser=localStorage.getItem('user'); if(savedUser){document.getElementById('waPhone').value=savedUser}
...
async def chat(req: Request):
    b=await req.json()
    return {"reply": ai_reply(b.get("message",""))}

@app.post("/user/login")
async def login_user(req: Request):
    b=await req.json()
    USERS[b.get("phone")]={"phone":b.get("phone"),"time":datetime.now().isoformat()}
    return {"success":True}
...
@app.get("/logo.png")
async def logo():
    return FileResponse("logo.png") if os.path.exists("logo.png") else {"error":"no logo"}
