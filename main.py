...
def smart_ai_reply(msg, cart_count=0):
    m=msg.lower()
    if "hello" in m or "hi" in m or "jambo" in m: return "Hello! I'm LONMA AI! What do you need?"
    if "help" in m: return "I can help:\n🛒 Find cheapest\n💰 Compare prices\n🏍️ Delivery 30min\n💳 Pay M-Pesa/Cash"
    if "deliver" in m: return "Yes! 30 mins Kajiado, Kitengela, Rongai. Fee KES 100."
    if "pay" in m: return "Payment:\n💚 M-Pesa STK Push\n💵 Cash on Delivery"
    if "terms" in m: return "📄 Terms: Delivery 30min, Fee KES 100, Returns 24h, M-Pesa + Cash. Full terms at /terms"
    return "Try: Help me, Will you deliver?, How to pay?, Terms"

def make_svg(text, emoji, bg="#FFFFFF"):
    svg = f'<svg xmlns="http://www.w3.org/2000/svg" width="300" height="300" viewBox="0 0 300 300"><rect width="300" height="300" fill="{bg}" rx="20"/><text x="150" y="130" font-size="80" text-anchor="middle">{emoji}</text><text x="150" y="220" font-size="16" font-weight="800" text-anchor="middle" fill="#0B0E14" font-family="Arial">{text}</text></svg>'
    return svg.encode()
...
<p><small>Last updated: Sept 16, 2026 - Kajiado, Kenya</small></p>
<div class="card"><h2>1. About Us</h2><p>LONMA ORBIT is grocery delivery serving Kajiado, Kitengela, Rongai. We connect you to Naivas, Quickmart, Carrefour, Chandarana, Magunas.</p></div>
<div class="card"><h2>2. Orders & Delivery</h2><ul><li>Delivery: 30min avg, max 60min. Fee KES 100 flat.</li><li>Free delivery first 3 orders code LONMA30.</li><li>Areas: Kajiado Town, Kitengela, Rongai, 15km radius.</li><li>Rider calls on arrival. Keep phone on.</li></ul></div>
<div class="card"><h2>3. Payments</h2><ul><li><b>M-Pesa STK Push:</b> Prompt on phone, enter PIN. Order confirmed after M-Pesa.</li><li><b>Cash on Delivery:</b> Pay rider exact. 5000+ bills inform rider.</li><li>Prices from supermarkets may change. Final at checkout.</li><li>Paybill 174379 (Sandbox).</li></ul></div>
<div class="card"><h2>4. Returns & Refunds</h2><ul><li>Perishables (milk, bread, tomatoes): Check at delivery. No returns after rider leaves unless wrong item.</li><li>Non-perishables (flour, Omo, soap, rice, Coke): Return 24h if sealed.</li><li>Wrong/damaged? WhatsApp with photo. Refund M-Pesa 24h.</li><li>No refund for traffic/weather/wrong location.</li></ul></div>
<div class="card"><h2>5. User Accounts</h2><ul><li>Login with WhatsApp. Keep phone secure.</li><li>One account per phone. Fake orders = ban + KES 500 penalty.</li></ul></div>
<div class="card"><h2>6. Privacy</h2><ul><li>We collect: Name, phone, location, cart. For delivery only.</li><li>No sharing except riders.</li><li>M-Pesa handled by Safaricom. No PIN stored.</li><li>Location deleted after 7 days.</li></ul></div>
<div class="card"><h2>7. Prohibited</h2><ul><li>No fake orders, no rider abuse, no false locations.</li></ul></div>
<div class="card"><h2>8. Contact</h2><p>WhatsApp: +254 7XX XXX XXX<br>Email: support@lonmaorbit.co.ke<br>Kajiado Town, Kenya<br><br>Complaint? Send Order ID e.g., ORD1234</p></div>
<div class="card" style="text-align:center"><p>By using LONMA ORBIT, you agree to these Terms.</p><p style="margin-top:10px"><b>LONMA ORBIT - 30 Min Grocery Delivery</b></p><p style="font-size:10px;margin-top:8px;color:#475569">© 2026 LONMA ORBIT • Kajiado</p></div>
</body></html>
    """)
...
<div class="header"><div class="h-top"><div class="logo">LONMA ORBIT</div><div class="icons"><div class="ic">🌙</div><div class="ic">🔍</div><div class="ic">🔔</div></div></div><div class="h-loc"><div class="loc-icon">📍</div><div><b>Kajiado Town • 30 min delivery</b><br><small id="userStatus">Guest • Login</small></div><div style="margin-left:auto;font-size:20px;opacity:0.6" onclick="openProfile()">›</div></div><div class="search-wrap"><div class="search-box">🔍<input id="search" placeholder="Search flour, milk, bread..." oninput="searchProd()"></div><div class="filter">☰</div></div></div>
...
<a href="/terms" style="color:#94A3B8;text-decoration:none">Contact Us</a>
</div>
<p style="font-size:10px;color:#475569;margin-top:10px">© 2026 LONMA ORBIT • Kajiado • 30min Delivery</p>
</div>
<div class="bottom"><div class="tab active"><div class="tab-i">🏠</div><b>Home</b></div><div class="tab"><div class="tab-i">📁</div><b>Categories</b></div><div class="tab" onclick="openCart()"><div class="tab-i">🛒</div><b>Cart</b><div class="cart-dot" id="cartDot">0</div></div><div class="tab" onclick="openRider()"><div class="tab-i">🏍️</div><b>Rider</b></div><div class="tab" onclick="openProfile()"><div class="tab-i">👤</div><b>Profile</b></div></div>
...
<input id="custName" class="input" placeholder="Full Name"><input id="custPhone" class="input" value="254" placeholder="M-Pesa Phone"><input id="custLoc" class="input" placeholder="Delivery Location">
<div style="margin-top:16px"><b style="font-size:13px">Choose Payment</b><div class="pay-methods"><div class="pay-opt active" id="payMpesa" onclick="setPay('mpesa')"><div style="font-size:22px">💚</div><b>M-Pesa</b><br><small>STK Push</small></div><div class="pay-opt" id="payCash" onclick="setPay('cash')"><div style="font-size:22px">💵</div><b>Cash</b><br><small>Pay rider</small></div></div></div>
...
<div id="cashStatus" class="pay-status cash">💵 Cash on Delivery<br><small>Pay KES <span id="cashAmount">0</span> to rider</small></div>
<div id="successStatus" class="pay-status success"><b>Order <span id="orderId"></span> Confirmed! 🎉</b><p style="font-size:11.5px;margin-top:6px" id="riderInfo"></p><p style="font-size:10px;margin-top:8px;opacity:0.8" id="paymentNote"></p></div>
<button class="btn btn-mpesa" id="placeBtn" onclick="checkout()">💚 Pay with M-Pesa - 30min</button><button class="btn btn-cash" id="cashBtn" onclick="checkoutCash()" style="display:none">💵 Place Order - Cash</button><div id="status" style="text-align:center;font-size:11px;font-weight:700;margin-top:10px;color:var(--muted)"></div></div></div></div>
...
<div id="profileModal" class="modal"><div class="sheet"><div class="s-h"><h3>Profile</h3><div onclick="closeM()" style="width:40px;height:40px;background:#0B0E14;border-radius:14px;display:flex;align-items:center;justify-content:center">✕</div></div><div class="s-c"><div style="text-align:center;padding:8px 0 22px"><div style="width:88px;height:88px;background:#0A8EA8;border-radius:28px;display:flex;align-items:center;justify-content:center;font-size:40px;color:#fff;margin:0 auto">👤</div><h3 style="margin-top:14px" id="profileName">Welcome</h3><small style="color:var(--muted)" id="profilePhone">Login to order faster</small></div><div id="notLogged"><input id="waPhone" class="input" value="254" placeholder="WhatsApp 254712..."><button class="btn btn-wa" onclick="loginWA()">Login with WhatsApp</button></div><div id="logged" style="display:none"><div style="background:#DCFCE7;border-radius:18px;padding:14px;text-align:center;color:#14532D"><b>Logged in ✓</b><br><small id="loggedPhone">254...</small></div><button class="btn" style="background:#fff;color:#000;margin-top:12px" onclick="logout()">Logout</button></div><div style="margin-top:16px"><button class="btn" style="background:#1A2035;color:#fff;border:1px solid var(--border)" onclick="window.open('/terms','_blank')">📄 Terms & Conditions</button><button class="btn" style="background:#1A2035;color:#fff;border:1px solid var(--border)" onclick="window.open('/terms','_blank')">🔒 Privacy Policy</button><p style="font-size:10px;color:#475569;text-align:center;margin-top:12px">© 2026 LONMA ORBIT • Kajiado Town • Version 2.1</p></div></div></div></div>
<div class="toast" id="toast"></div>
<script>
var PRODUCTS=[{id:1,name:"Ajab Maize Flour 2kg",price:175,old:195,store:"Naivas",cat:"grocery",rate:4.8,sold:234,img:"/img/flour",emoji:"🌽"},{id:2,name:"Brookside Milk 500ml",price:65,old:75,store:"Naivas",cat:"dairy",rate:4.9,sold:512,img:"/img/milk",emoji:"🥛"},{id:3,name:"Coca Cola 1.25L",price:100,old:120,store:"Quickmart",cat:"drinks",rate:4.7,sold:320,img:"/img/coke",emoji:"🥤"},{id:4,name:"Omo Detergent 1kg",price:285,old:320,store:"Carrefour",cat:"home",rate:4.6,sold:89,img:"/img/omo",emoji:"🧴"},{id:5,name:"Tomatoes Fresh 1kg",price:80,old:100,store:"Quickmart",cat:"fresh",rate:4.9,sold:445,img:"/img/tomato",emoji:"🍅"},{id:6,name:"White Bread 400g",price:60,old:70,store:"Naivas",cat:"dairy",rate:4.8,sold:210,img:"/img/bread",emoji:"🍞"},{id:7,name:"Pishori Rice 2kg",price:350,old:400,store:"Carrefour",cat:"grocery",rate:4.9,sold:156,img:"/img/rice",emoji:"🍚"},{id:8,name:"Geisha Soap 150g",price:55,old:65,store:"Magunas",cat:"care",rate:4.5,sold:98,img:"/img/soap",emoji:"🧼"}];
var STORES=["ALL","Naivas","Quickmart","Carrefour","Chandarana","Magunas"];
var CATS=[{id:"all",name:"All",icon:"🏬"},{id:"fresh",name:"Fresh",icon:"🥬"},{id:"grocery",name:"Grocery",icon:"🌽"},{id:"drinks",name:"Drinks",icon:"🥤"},{id:"dairy",name:"Dairy",icon:"🥛"}];
...
function loginWA(){var phone=document.getElementById("waPhone").value;if(phone.length<10){alert("Enter valid number");return;}localStorage.setItem("user",phone);document.getElementById("notLogged").style.display="none";document.getElementById("logged").style.display="block";document.getElementById("loggedPhone").innerText=phone;document.getElementById("profileName").innerText="Welcome! "+phone.slice(-4);document.getElementById("profilePhone").innerText=phone;document.getElementById("userStatus").innerText="Logged in • "+phone;document.getElementById("custPhone").value=phone;showToast("Logged in ✅");}
...
async def grocery_icon():
    return Response(content=make_svg("GROCERY","🌽","#0A8EA8"), media_type="image/svg+xml")

@app.get("/img/{name}")
async def product_img(name: str):
    maps={"flour":("AJAB FLOUR","🌽"),"milk":("BROOKSIDE","🥛"),"coke":("COCA COLA","🥤"),"omo":("OMO 1KG","🧴"),"tomato":("TOMATOES","🍅"),"bread":("BREAD 400G","🍞"),"rice":("PISHORI","🍚"),"soap":("GEISHA","🧼")}
