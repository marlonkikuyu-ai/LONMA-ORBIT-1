...
def ai_reply(m):
    m=m.lower()
    if "flour" in m: return "Ajab Flour 2kg KES 175 Naivas, 172 Carrefour. 50 packs!"
    if "milk" in m: return "Brookside Milk KES 65 Naivas, 62 Chandarana!"
    if "rider" in m: return "Delivery 30min Kajiado/Kitengela/Rongai. Fee KES 100. 3 riders."
    if "dark" in m: return "Click moon top right for dark mode!"
    if "hello" in m or "hi" in m: return "Hello! I am LONMA AI. Ask cheapest flour or milk price."
    return "Try: cheapest flour, milk price, rider time"
...
.logo{width:110px;height:36px;background:#0A8EA8;border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:900;font-size:14px;letter-spacing:1px}
...
.card-body{padding:10px}.store{font-size:8px;font-weight:800;color:#0A8EA8}.card-body h4{font-size:12px;font-weight:700;line-height:1.3;margin:3px 0;height:32px;overflow:hidden}.meta{font-size:10px;color:var(--muted);margin:4px 0}.price-row{display:flex;justify-content:space-between;align-items:center;margin-top:8px}.price b{font-size:14px}.price small{font-size:10px;color:#999;text-decoration:line-through;margin-left:4px}.add-btn{width:32px;height:32px;background:#111;color:#fff;border:none;border-radius:11px;font-size:18px;font-weight:800;cursor:pointer}
.h-scroll{display:flex;gap:10px;overflow-x:auto;padding-bottom:6px}.h-card{min-width:160px;background:var(--card);border-radius:18px;padding:10px;border:1px solid var(--border)}
...
.ai-h{background:#0A8EA8;color:#fff;padding:14px 16px;display:flex;justify-content:space-between;align-items:center}.ai-msgs{flex:1;overflow-y:auto;padding:14px;display:flex;flex-direction:column;gap:10px;background:var(--bg)}.m{max-width:80%;padding:10px 14px;border-radius:18px;font-size:12px}.m.u{align-self:flex-end;background:#0A8EA8;color:#fff}.m.b{align-self:flex-start;background:var(--card);border:1px solid var(--border)}.ai-in{display:flex;gap:8px;padding:12px;border-top:1px solid var(--border);background:var(--card)}.ai-in input{flex:1;padding:12px 16px;border-radius:24px;border:1px solid var(--border);background:var(--bg);color:var(--text)}.ai-in button{padding:12px 18px;background:#0A8EA8;color:#fff;border:none;border-radius:24px;font-weight:800}
...
<div class="hero"><div><h2>Free Delivery<br>on First 3 Orders!</h2><p>Use code LONMA30 • Real deals</p></div><div class="hero-btn">ORDER NOW</div></div>
...
<div id="aiChat"><div class="ai-h"><div><b>LONMA AI</b><div style="font-size:11px;opacity:0.85">Online • 5 stores</div></div><div onclick="toggleAI()" style="width:32px;height:32px;background:rgba(255,255,255,0.2);border-radius:10px;display:flex;align-items:center;justify-content:center;cursor:pointer">✕</div></div><div class="ai-msgs" id="aiMsgs"><div class="m b">Hello! Ask cheapest flour, milk price, rider time</div></div><div class="ai-in"><input id="aiInput" placeholder="Ask anything..." onkeypress="if(event.key==='Enter') sendAI()"><button onclick="sendAI()">Send</button></div></div>
...
function renderChips(){
 ...
 for(var i=0;i<STORES.length;i++){
   var s = STORES[i];
   var cls = s===activeStore? "chip active" : "chip";
   sHtml += '<div class="'+cls+'" onclick="setStore(\\''+s+'\\')">'+s+'</div>';
 }
 document.getElementById("storeChips").innerHTML = sHtml;
 var cHtml = "";
 for(var j=0;j<CATS.length;j++){
   var c = CATS[j];
   var cl = c.id===activeCat? "cat active" : "cat";
   cHtml += '<div class="'+cl+'" onclick="setCat(\\''+c.id+'\\')"><div class="cat-icon">'+c.icon+'</div><b>'+c.name+'</b></div>';
 }
...
function renderFlash(){
 ...
 for(var i=0;i<4;i++){
   var p = PRODUCTS[i];
   html += '<div class="h-card"><div style="font-size:32px;text-align:center;padding:10px">'+p.emoji+'</div><div style="font-size:11px;font-weight:700">'+p.name+'</div><div style="display:flex;justify-content:space-between;align-items:center;margin-top:8px"><b>KES '+p.price+'</b><button class="add-btn" style="width:26px;height:26px;font-size:14px" onclick="addToCart('+p.id+')">+</button></div></div>';
 }
...
function filterProducts(){
 ...
 for(var i=0;i<PRODUCTS.length;i++){
   var p = PRODUCTS[i];
   var storeMatch = activeStore==="ALL" || p.store===activeStore;
   var catMatch = activeCat==="all" || p.cat===activeCat;
   if(storeMatch && catMatch) filtered.push(p);
 }
...
function searchProd(){
 ...
 for(var i=0;i<PRODUCTS.length;i++){
   var p = PRODUCTS[i];
   if(p.name.toLowerCase().indexOf(q)>-1 || p.store.toLowerCase().indexOf(q)>-1) f.push(p);
 }
 renderProducts(f);
}
...
function openCart(){
 ...
 if(cart.length===0){ d.innerHTML = '<p style="text-align:center;padding:20px;color:var(--muted)">Cart empty - add items</p>'; }
 else {
   var html = "";
   for(var i=0;i<cart.length;i++){
     var c = cart[i];
     html += '<div class="cart-i"><div class="ci">'+c.emoji+'</div><div style="flex:1"><h4 style="font-size:12px">'+c.name+'</h4><small>'+c.store+' • KES '+c.price+'</small></div><b>KES '+c.price+'</b></div>';
   }
   d.innerHTML = html;
 }
 document.getElementById("sub").innerText = total;
 document.getElementById("grand").innerText = total+100;
...
function loginWA(){
 ...
 document.getElementById("loggedPhone").innerText=phone;
 document.getElementById("profileName").innerText="Welcome! "+phone.slice(-4);
 document.getElementById("profilePhone").innerText=phone;
 document.getElementById("userStatus").innerText="Logged in • "+phone;
 document.getElementById("custPhone").value=phone;
 showToast("Logged in via WhatsApp ✅");
 fetch("/user/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({phone:phone})});
}
function logout(){ localStorage.removeItem("user"); document.getElementById("notLogged").style.display="block"; document.getElementById("logged").style.display="none"; document.getElementById("profileName").innerText="Welcome"; document.getElementById("profilePhone").innerText="Login to order faster"; document.getElementById("userStatus").innerText="Guest • Login with WhatsApp"; showToast("Logged out"); }
...
async function checkout(){
 ...
 var loc = document.getElementById("custLoc").value;
 ...
 document.getElementById("riderInfo").innerText="Rider John KMEZ 123A • 4.9 stars • 2.5km away • 30min";
...
async function loadAdmin(){
 ...
 for(var i=0;i<Math.min(o.length,8);i++){
   var x = o[i];
   html += '<div style="background:var(--card);border:1px solid var(--border);border-radius:14px;padding:10px;margin-bottom:6px"><b>'+x.id+'</b> - '+x.status+'<br><small>'+x.location+' • KES '+x.amount+'</small></div>';
 }
...
var savedUser = localStorage.getItem("user"); if(savedUser){ document.getElementById("waPhone").value=savedUser; }
...
setInterval(function(){
 ...
 if(el) el.innerText = "Ends "+ (h<10?"0"+h:h) + ":" + (m<10?"0"+m:m) + ":" + (s<10?"0"+s:s);
},1000);
</script></body></html>
''')
...
async def chat(req: Request):
    ...
    reply="Try: cheapest flour, milk price, rider time"
    if "flour" in msg: reply="Ajab Flour 2kg KES 175 Naivas, 172 Carrefour. 50 packs!"
    elif "milk" in msg: reply="Brookside Milk KES 65 Naivas, 62 Chandarana!"
    elif "rider" in msg: reply="Delivery 30min Kajiado/Kitengela/Rongai. Fee KES 100. 3 riders online."
    elif "hello" in msg or "hi" in msg: reply="Hello! I am LONMA AI. Ask cheapest flour or milk price."
    return {"reply": reply}
